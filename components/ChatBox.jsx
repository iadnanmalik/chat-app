...
const ChatBox = (props) => {
  ...
  useEffect(() => {
    if (props.user.name) {
      const socketRef = connect(props.user.name);
          ...
          <MessageListColumn>
            {messageList?.map((message) => {
              if (
                ...
                (message.from === props.user.name &&
                  message.to === selectedContact)
              )
                return (
                  <MessageWrapper
                    key={`${message.message}${message.to}${message.from}`}
                  >
                    {message.from === props.user.name ? (
                      <MessageRight>
                        {message.message}
...
export const ChatBoxComponent = withAuth(ChatBox);
